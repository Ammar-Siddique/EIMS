...
if(isset($_POST['save']))
    {
        ...
        {
            if($roll_no == $row['roll_no'] && $course_id == $row['course_id'])
            {
                $sql2 = "DELETE FROM `eims`.`students_courses` WHERE `students_courses`.`roll_no` = '$roll_no' and `students_courses`.`course_id` = '$course_id';";
        ...
        // Flag for successful insertion
                    ...
                    $sql3 = "DELETE FROM `eims`.`marks` WHERE `marks`.`roll_no` = '$roll_no' and `marks`.`course_id` = '$course_id';";
...
<div class="sidebar">
    ...
    <br>
        <form action="withdraw course.php" method="POST">

            <label for="course_id">Enter Course id:</label>
            <input type="course_id" id="course_id" name="course_id" required>
            <br><br>
    
            <label for="roll_no">Enter your Roll#:</label>
            <input type="roll_no" id="roll_no" name="roll_no" required>
            <br><br>
    
            <!-- <input type="submit" value="Withdraw" name="save"> -->
            <input class= "btn btn-danger" type="submit"  value="Submit" name="save" >   
            <?php
        if($flag == true)
        {
            echo "<br><p>This course is not selected!</p><br>";
        }
        
        if($insert2 == true){
        echo "<br><p>Course withdrawn.</p>";
        }
    ?>
        </form>
    </div>
